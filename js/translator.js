...
const IBM_Watson_selected_Value = target_IBM_Watson_Select.value;
...
//Node.js 필요
async function detectLanguageWithPapago(text) {
    const papago_client_id = 'ogwo51ja4q';
    const papago_apiKey = 'pObTOdYCeFpFbyfvwb4dDrNNLkzuZ7EbyJrjBSAP';
    const papago_url = 'https://naveropenapi.apigw.ntruss.com/nmt/v1/translate';
    ...
    try{
        ...
        return data.message.result.langCode;
    } catch (error) {
...
async function translateWithMicrosoft(text, sourceLang, targetLang) {
    const microsoft_apiKey = 'nxpnJSDSjMlIOLah42fnkSqng3VMCuMyX8AM86B6Dv6FvSGmwwtuJQQJ99ALACNns7RXJ3w3AAAbACOGrkr3';
    const microsoft_url = 'https://api.cognitive.microsofttranslator.com/translate?api-version=3.0';
    ...
    console.log('Request URL:', microsoft_url); // 요청 URL 로그 출력
    console.log('Request Body:', JSON.stringify(requestBody)); // 요청 본문 로그 출력
    console.log('API Key:', microsoft_apiKey); // API 키 로그 출력
    console.log('Region:', 'koreacentral'); // 지역 확인
...
// 사용 예:
//translateWithDeepL(inputText.value, 'EN'); // 한국어 -> 영어 번역

// select 값 변경 감지하기
// targetdeeplSelect.addEventListener('change', (event) => {
//     const selectedValue = event.target.value;
//     console.log('선택된 언어:', selectedValue);
    
//     // 여기서 번역 함수 호출
//     translateWithDeepL(inputText.value, selectedValue);
// });
